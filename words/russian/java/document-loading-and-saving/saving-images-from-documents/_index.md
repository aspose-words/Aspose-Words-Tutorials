...
Чтобы использовать обратный вызов для настройки сохранения изображения, установите `PageSavвgCallback` in `ImageSaveOptions`. Создайте класс, реализующий `IPageSavingCallback` интерфейс и переопределить `pageSaving` метод.
