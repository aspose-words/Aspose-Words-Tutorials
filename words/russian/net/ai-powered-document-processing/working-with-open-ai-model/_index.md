...
Теперь, когда ваши документы загружены, пришло время вытащить ваш ключ API OpenAI. Вы получите его из переменных среды, чтобы сохранить его в безопасности:
```csharp
...
Так чего же вы ждете? Погрузитесь в свои документы с уверенностью и позвольте технологиям сделать тяжелую работу!
