...
Первым делом нам нужно загрузить документ Word, с которым мы собираемся работать. Вот где Aspose.Words блистает, позволяя вам легко программно манипулировать документами Word.
...
Здесь мы извлекаем первую таблицу в документе. `GetChild` метод используется для поиска узла таблицы, с `NodeType.Table` указав тип узла, который мы ищем. `0` указывает, что нам нужна первая таблица, и `true` гарантирует, что мы проведем поиск по всему документу.
