...
Прежде чем мы выполним замену текста, нам нужно настроить некоторые параметры для операции поиска и замены. Это включает в себя чувствительность к регистру и необходимость сопоставления только целых слов.
...
В этом примере `MatchCase` установлен на `false` игнорировать различия в регистрах и `FindWholeWordsOnly` установлен на `false` чтобы разрешить частичные совпадения внутри слов.
...
Вы можете заменить любой определенный текст в нижнем колонтитуле. Если вам нужно заменить несколько строк, убедитесь, что ваша строка поиска точно соответствует тексту, который вы хотите заменить.
