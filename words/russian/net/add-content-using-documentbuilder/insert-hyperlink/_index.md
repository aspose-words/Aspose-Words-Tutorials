...
Сначала мы импортируем необходимые пространства имен. Это важно, поскольку позволяет нам получить доступ к классам и методам, необходимым для манипуляции документами.
...
Для начала нам нужно определить путь к каталогу наших документов. Это место, где будет сохранен наш документ Word.
...
Используя `DocumentBuilder`мы напишем начальный текст в документ. Это задаст контекст, в который будет вставлена наша гиперссылка.
...
Теперь мы вставляем гиперссылку с помощью `InsertHyperlink` метод. Этот метод принимает три параметра: отображаемый текст, URL и логическое значение, указывающее, следует ли форматировать ссылку как гиперссылку.
