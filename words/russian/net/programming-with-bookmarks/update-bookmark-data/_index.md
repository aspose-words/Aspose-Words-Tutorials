...
Представьте себе закладку как флаг, отмечающий определенное место в вашем документе. Чтобы изменить его содержимое, нам нужно сначала его найти. Aspose.Words предлагает `Bookmarks` коллекция в пределах `Range` объект, позволяющий вам получить определенную закладку по ее имени. Вот как мы это делаем:
