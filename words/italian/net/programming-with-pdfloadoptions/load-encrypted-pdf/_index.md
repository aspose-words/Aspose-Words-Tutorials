...
1. Aspose.Words per .NET: se non lo hai già fatto, scaricalo [Qui](https://releases.aspose.com/words/net/).
2. Una licenza valida: per accedere a tutte le funzionalità senza limitazioni, valuta l'acquisto di una licenza [Qui](https://purchase.aspose.com/buy)In alternativa, puoi utilizzare un [licenza temporanea](https://purchase.aspose.com/temporary-license/).
...
Sostituire `"YOUR DOCUMENT DIRECTORY"` Con il percorso effettivo in cui sono archiviati i file PDF. Questo sarà l'area di lavoro per i file PDF.
...
Qui creiamo un nuovo `PdfSaveOptions` oggetto e imposta il suo `EncryptionDetails`La password `"Aspose"` viene utilizzato per crittografare il PDF.
...
Qui creiamo un nuovo `PdfLoadOptions` Inserisci la password e carica il documento PDF crittografato. Ecco fatto! Il tuo PDF crittografato è ora caricato e pronto per ulteriori elaborazioni.
...
Sì, basta sostituirlo `"Aspose"` con la password desiderata nel `PdfEncryptionDetails` oggetto.
