---
"description": "Lär dig hur du arbetar med fotnoter och slutnoter i dina Word-dokument med Aspose.Words för .NET. Detaljerade handledningar och praktiska exempel."
"linktitle": "Ordbehandling med fotnot och slutnot"
"second_title": "Aspose.Words dokumentbehandlings-API"
"title": "Ordbehandling med fotnot och slutnot"
"url": "/sv/net/working-with-footnote-and-endnote/"
"weight": 660
---

{{< blocks/products/pf/main-wrap-class >}}

{{< blocks/products/pf/main-container >}}

{{< blocks/products/pf/tutorial-page-section >}}

# Ordbehandling med fotnot och slutnot

Ordbehandling med fotnoter och slutnoter är en omfattande resurs som guidar dig genom ordbehandling med fotnoter och slutnoter i dina Word-dokument med hjälp av Aspose.Words-biblioteket för .NET. Handledningarna ger detaljerade steg-för-steg-förklaringar, fullständig källkod och praktiska exempel för att lägga till, ändra och ta bort fotnoter och slutnoter. Du lär dig också hur du anpassar format och stil på anteckningar, navigerar mellan anteckningar och konverterar dem till olika format, till exempel PDF eller HTML. Denna resurs är viktig för utvecklare som vill hantera referenser och anteckningar i sina Word-dokument med hjälp av Aspose.Words för .NET.

 ## Handledningar
| Titel | Beskrivning |
| --- | --- |
| [Ställ in fotnotskolumner](./set-foot-note-columns/) | Lär dig hur du ställer in fotnotskolumner i Word-dokument med Aspose.Words för .NET. Anpassa enkelt din fotnotslayout med vår steg-för-steg-guide. |
| [Ställ in fotnots- och slutnotsposition](./set-footnote-and-end-note-position/) Lär dig hur du ställer in fotnots- och slutnotspositioner i Word-dokument med Aspose.Words för .NET med den här detaljerade steg-för-steg-guiden. |
| [Ange alternativ för slutnot](./set-endnote-options/) | Lär dig hur du ställer in alternativ för slutnoter i Word-dokument med Aspose.Words för .NET med den här omfattande steg-för-steg-guiden. |
| [Hämta slutnotseparator vid inläsning av ett Word-dokument i C#](./retrieve-endnote-separator-while-loading-a-word-document-in/) | Lär dig hur du hämtar separatorn för slutnoter när du laddar ett Word-dokument i C# med Aspose.Words för .NET. |

{{< /blocks/products/pf/tutorial-page-section >}}


{{< /blocks/products/pf/main-container >}}

{{< /blocks/products/pf/main-wrap-class >}}


{{< blocks/products/products-backtop-button >}}