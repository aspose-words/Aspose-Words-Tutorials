...
description: How to add data labels in C# with Aspose.Words. Learn to edit chart,
  center chart data labels, show percentages in chart, and customize chart data labels.
draft: false
images:
...
language: en
lastmod: 2026-08-04
og_description: How to add data labels in C# using Aspose.Words. This tutorial shows
  you how to edit chart, center chart data labels, show percentages in chart, and
  customize chart data labels.
og_image_alt: Screenshot of a Word chart with data labels added using C#
og_title: How to add data labels to a Word chart in C# – complete guide
schemas:
- author: Aspose
...
- C#
- Chart manipulation
title: How to add data labels to a Word chart in C# – step‑by‑step guide
url: /net/programming-with-charts/how-to-add-data-labels-to-a-word-chart-in-c-step-by-step-gui/
---
...
# How to add data labels to a Word chart in C# – step‑by‑step guide

If you need to **how to add data labels** to a chart that lives inside a Word document, this guide shows you the exact code you must run. You’ll see how to edit chart properties, center chart data labels, show percentages in chart, and customize chart data labels for any scenario.

The tutorial covers everything required to modify an existing chart, from loading the document to persisting the changes. No external references are needed—just the Aspose.Words for .NET library and a basic C# development environment.
...
| Pitfall | Why it happens | Recommended fix |
|---|---|---|
| The chart is inside a grouped shape. | `GetChild(NodeType.Shape, …)` returns the outer group, not the inner chart. | Search recursively for a shape with `shape.HasChart`. |
| Data labels do not appear after saving. | `ShowValue` or `ShowPercentage` was not set to `true`. | Explicitly set both `ShowValue` and `ShowPercentage` as needed. |
| Labels overlap on small slices. | Center positioning can cause crowding. | Use `ChartDataLabelPosition.OutSideEnd` for outside placement, or enable `LeaderLines`. |
