...
Teraz omówimy proces modyfikacji formatowania komórek na proste, łatwe do wykonania kroki.
...
Następnie musisz uzyskać dostęp do tabeli w dokumencie. Pomyśl o tym jak o wizualnym zlokalizowaniu tabeli w dokumencie, ale my robimy to za pomocą kodu.

```csharp
Table table = (Table)doc.GetChild(NodeType.Table, 0, true);
```

Tutaj używamy `GetChild` metoda pobierania pierwszej tabeli w dokumencie. `NodeType.Table` parametr określa, że szukamy tabeli i `0` wskazuje pierwszą tabelę. `true` Parametr zapewnia głębokie przeszukiwanie, co oznacza, że przeszukane zostaną wszystkie węzły podrzędne.
