...
Aspose.Words for .NET 是一个功能强大的库，允许您以编程方式创建、操作和转换 Word 文档。您可能需要执行的一项常见任务是检索文档中特定字体的行距。在本教程中，我们将逐步指导您完成该过程，确保您能够使用 Aspose.Words for .NET 轻松获取字体行距。 
...
2. 开发环境：确保您已设置类似 Visual Studio 的 IDE。
3. C# 基础知识：本教程假设您对 C# 编程有基本的了解。

## 导入命名空间

首先，您需要在 C# 项目中导入必要的命名空间。这些命名空间将允许您访问 Aspose.Words 的功能。
...
现在，我们设置要插入文本的字体属性。在本例中，我们将使用“Calibri”字体。
...
使用 `DocumentBuilder` 对象，在文档中写入一些文本。这些文本将使用我们在上一步中设置的字体属性。
...
要获取行距，我们需要访问刚刚添加的文本的字体对象。这可以通过导航到文档结构中的第一个段落来完成。
...
就是这样！使用 Aspose.Words for .NET 获取字体行距非常简单，只需将其分解为以下几个简单的步骤即可。无论您是创建新文档还是使用现有文档，Aspose.Words 都能提供高效管理字体属性所需的所有工具。
...
您可以从以下位置下载最新版本的 Aspose.Words for .NET [这里](https://releases。aspose.com/words/net/).
