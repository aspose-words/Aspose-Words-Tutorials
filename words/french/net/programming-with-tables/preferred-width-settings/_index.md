---
"description": "Découvrez comment créer des tableaux avec des paramètres de largeur absolue, relative et automatique dans Aspose.Words pour .NET avec ce guide étape par étape."
...
Les tableaux sont un moyen efficace d'organiser et de présenter les informations dans vos documents Word. Lorsque vous travaillez avec des tableaux dans Aspose.Words pour .NET, vous disposez de plusieurs options pour définir la largeur des cellules afin de garantir une adaptation parfaite à la mise en page de votre document. Ce guide vous guidera dans la création de tableaux avec des paramètres de largeur prédéfinis avec Aspose.Words pour .NET, en mettant l'accent sur les options de dimensionnement absolu, relatif et automatique. 

## Prérequis

Avant de plonger dans le didacticiel, assurez-vous de disposer des éléments suivants :

1. Aspose.Words pour .NET : Assurez-vous d'avoir installé Aspose.Words pour .NET dans votre environnement de développement. Vous pouvez le télécharger. [ici](https://releases.aspose.com/words/net/).
...
Insérez la première cellule du tableau avec une largeur fixe de 40 points. Cela garantira que cette cellule conservera toujours une largeur de 40 points, quelle que soit la taille du tableau.
...
Dans cette étape, vous commencez à créer le tableau et insérez une cellule avec une largeur absolue. `PreferredWidth.FromPoints(40)` la méthode définit la largeur de la cellule à 40 points, et `Shading.BackgroundPatternColor` applique une couleur d'arrière-plan jaune clair.

## Étape 3 : Insérer une cellule de taille relative

Insérez une autre cellule dont la largeur correspond à 20 % de la largeur totale du tableau. Ce dimensionnement relatif garantit que la cellule s'ajuste proportionnellement à la largeur du tableau.
...
Enfin, insérez une cellule qui se dimensionne automatiquement en fonction de l’espace disponible restant dans le tableau.
...
builder.Writeln("Cell automatically sized. Le size of this cell is calculated from the table preferred width.");
builder.Writeln("In this case the cell will fill up the rest of the available space.");
```

The `PreferredWidth.Auto` Ce paramètre permet à cette cellule de s'agrandir ou de se réduire en fonction de l'espace restant après la prise en compte des autres cellules. Cela garantit une mise en page du tableau équilibrée et professionnelle.
...
Non, les pourcentages négatifs ne sont pas valides pour la largeur des cellules. Seuls les pourcentages positifs sont autorisés.
