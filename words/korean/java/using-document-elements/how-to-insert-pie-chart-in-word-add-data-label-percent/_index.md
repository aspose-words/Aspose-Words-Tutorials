...
category: general
date: 2026-07-20
description: Aspose.Words를 사용하여 Word에 파이 차트를 삽입하는 방법. 데이터 레이블 백분율을 추가하고 차트에 백분율을 표시하는
  방법을 배워 전문 문서를 만들세요.
draft: false
images:
...
language: ko
lastmod: 2026-07-20
og_description: Aspose.Words를 사용하여 Word에 파이 차트를 삽입하는 방법. 이 가이드는 데이터 레이블 백분율을 추가하고
  차트에 백분율을 표시하는 방법을 몇 줄만으로 보여줍니다.
og_image_alt: Screenshot showing how to insert pie chart in Word with percentage labels
og_title: Word에서 원형 차트 삽입 방법 – 빠른 가이드
schemas:
- author: Aspose
...
- Chart
- Word Automation
title: Word에서 파이 차트 삽입 방법 – 데이터 레이블에 백분율 추가
url: /ko/java/using-document-elements/how-to-insert-pie-chart-in-word-add-data-label-percent/
---
...
# Word에서 파이 차트 삽입 방법 – 데이터 레이블 백분율 추가

Ever wondered **파이 차트 삽입 방법** into a Word document without wrestling with the UI? You’re not alone. In many reporting scenarios you need to *add pie chart to Word* and, more importantly, **show percent on pie chart** so readers instantly grasp the data distribution.
...
![Screenshot of a Word document containing a pie chart with percentage labels](/images/pie-chart-percent.png){.center width=600px alt="파이 차트에 백분율 레이블이 포함된 Word 문서 스크린샷"}
