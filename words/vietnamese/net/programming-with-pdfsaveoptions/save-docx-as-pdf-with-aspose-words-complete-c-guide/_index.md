...
## Bước 4 – Xác minh đầu ra (Tùy chọn)
...
## Các trường hợp Edge thường gặp & Cách xử lý
...
## Cách chuyển đổi Word sang PDF trong một dòng (Phần thưởng)
...
**Hỏi: Tính năng này có hoạt động với tệp .doc hay chỉ .docx?**
A: Có. Aspose.Words hỗ trợ cả `.doc` Legacy và `.docx` hiện đại. Chỉ cần trỏ `sourcePath` tới tệp phù hợp.

**Q: Nếu tôi cần ẩn hoàn toàn các hình dạng nổi thì sao?**
A: Đặt `ExportFloatingShapesAsInlineTag = false` (mặc định) và tùy chọn loại bỏ chúng khỏi tài liệu trước khi lưu.

**Q: Tôi có thể thêm mật khẩu vào tệp PDF được tạo không?**
A: Chắc chắn. Sử dụng `pdfOptions.EncryptionDetails = new PdfEncryptionDetails("userPwd", "ownerPwd", PdfPermissions.All);`

**Q: Có cách nào để chuyển đổi toàn bộ thư mục chứa tệp DOCX không?**
A: Bao bọc mã chuyển đổi trong một vòng `foreach (var file in Directory.GetFiles(folder, "*.docx"))`. Tái sử dụng cùng một thể hiện `PdfSaveOptions` sẽ cải thiện hiệu suất.
